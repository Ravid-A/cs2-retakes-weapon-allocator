#!/usr/bin/env python3
"""
Generate alloc_menu.vxml and weapon_icons.vcss from one slot count.
...
xml = f'''<!--
  ...
  `wi-ak47` on the tile picks the icon up through `.wi-ak47 .am-ico` in weapon_icons.vcss, so a
...
css = f'''/* ============================================================================
   weapon_icons - one class per weapon the allocator can hand out. GENERATED by
...
(ROOT / "panorama/layout/custom_game/alloc_menu.vxml").write_text(xml, encoding="utf-8")
(ROOT / "panorama/styles/custom_game/weapon_icons.vcss").write_text(css, encoding="utf-8")
print(f"alloc_menu.vxml: {N} tiles x 4 rows + 3 AWP, card {card_w}px")
print(f"weapon_icons.vcss: {len(WEAPONS)} weapon classes")
